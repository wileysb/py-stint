...
\attention
The ECMWF server sets a limit of \~2GB per file download.  Data should be downloaded in netcdf format, in either 6-hour or 24-hour resolution, with a single field (attribute) per file. 
...
MODIS products can be downloaded from two main sources, given below.  Future versions of py-stint will incorporate dataset retrieval tools and functions based on the downmodis module from the project pyModis, however for now the 
...
* Identify \texttt{MODIS} tiles overlapping \texttt{AOI}

* Download \texttt{MODIS} products for identified tiles

* Arrange \texttt{MODIS\_Archive/h\_\_v\_\_/*.hdf} structure
